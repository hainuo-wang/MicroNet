...
from util import read_split_data, train_one_epoch, evaluate, plot_accuracy
...
def main(args):
    ...
    # exit()
    # 如果存在预训练权重则载入
    model = MicroNet(cfg, input_channels=3, num_classes=args.num_classes).to(device)
    if args.weights != "":
    ...
    plot_accuracy(args.epochs, total_accuracy_train, total_accuracy_val, "MicroNet_KMU_FED")
