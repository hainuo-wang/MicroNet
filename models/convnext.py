...
class GlobalSparseAttn():
    def __init__(self, dim, sample_rate, scale):
        ...
        self.LocalProp = nn.ConvTranspose2d(dim, dim, kernel_size, stride=sample_rate, groups=dim
        )
...
class DownSampleLayer():
    def __init__(self, dim_in, dim_out, downsample_rate):
        self.downsample = nn.Conv2d(dim_in, dim_out, kernel_size=downsample_rate, stride=
        downsample_rate)
...
class PatchEmbed():
    def __init__(self, dim):
        self.embed = nn.Conv2d(dim, dim, 3, padding=1, groups=dim)
    def forward(self, x):
