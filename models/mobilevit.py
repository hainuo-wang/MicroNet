...
model_urls = {
    ...
    "mobilevit_s": "https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-mvit-weights/mobilevit_s-38a5a959.pth",
}
...
def mobilevit_xs(pretrained=False,**kwargs):
    ...
    model=MobileViT((256, 256), dims, channels, num_classes=1000)
    if pretrained:
        url = model_urls['mobilevit_xs']
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu", check_hash=True)
        print(checkpoint)
        model.load_state_dict(checkpoint["model"])
    return model
